...
unsigned char state_machine(void){
	...
	switch(CMDCommand){
		...
		case PICTURE_CMD:
			...
			// check if Socket video is already created
			if(bVideo != VIDEO_OK){
...
void* Thread_Read_Command(){
	...
	while(CMDCommand != DISCONN_CMD){
		CMDCommand = read_socket(newSockData);
	}
...
void* Thread_Send_Data_PC(){
	...
	while(iStopCommand != 1){
		...
		strInputStatus Mess;
		libcom_InputStatus(&Mess, ptrCaptorMainDoor->stateCapt, ptrCaptorMainDoor->stateCapt, stateInterrupter, temperature,humidity);
